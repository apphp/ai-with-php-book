...
class ChunkedProcessor {
    ...
    public function __construct($chunkSize = 1000, $maxMemoryUsage = '256M', LogHandler $logHandler) {
    ...
    public function processLargeDataset($filename, callable $processor) {
        try {
            ...
            ini_set('memory_limit', $this->maxMemoryUsage);
            ...
            if ($handle === false) {
                throw new FileOpenException("Failed to open file: $filename");
            }
            ...
            while (!feof($handle)) {
                try {
                    ...
                    $stats['memory_peak'] = max($stats['memory_peak'], 
            ...
            throw new ProcessingException("Dataset processing failed: " . $e->getMessage());
        }
    }
